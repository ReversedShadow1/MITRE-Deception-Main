...
class EnhancedExtractionManager:
    ...
    def __init__(
        ...
        # Initialize database connection - ADD THIS LINE
        from src.database.postgresql import get_db
    ...
    '''# Add to src/api/v2/routes/Enhanced_Extraction_Manager.py
    def _extract_sync(
        self,
        text: str,
        extractors: List[str],
        ...
        top_k: int,
        use_ensemble: bool,
        job_id: str = None  # Added job_id parameter
    ) -> Dict:
        """
        ...
        for extractor_name in extractors:
            ...
            if extractor_name in ["rule_based", "ner"]:
                ...
                # Use classifier-specific method
                results = extractor.classify_techniques(
                    text=text,
                    threshold=threshold,
                    top_k=top_k,
                    job_id=job_id  # Pass job_id for metrics recording
        ...
        if use_ensemble and extractor_results:
            ...
            # Record ensemble details if job_id provided
            if job_id and use_ensemble and techniques:
                from src.database.metrics_recorder import MetricsRecorder
                metrics_recorder = MetricsRecorder(job_id)
                
                for technique in techniques:
                    if technique.get("method") == "ensemble" and "component_scores" in technique:
                        # Record ensemble details
                        metrics_recorder.record_ensemble_details(
                            technique_id=technique.get("technique_id", "unknown"),
                            ensemble_method="advanced_ensemble",
                            final_confidence=technique.get("confidence", 0.0),
                            component_scores=technique.get("component_scores", {}),
                            weights_used=self.ensemble.base_weights  # Get weights from ensemble object
                        )
        else:
    ...
    def _extract_async(
        ...
        """
        Extract techniques asynchronously
    ...
    # Add this to EnhancedExtractionManager in src/api/v2/routes/Enhanced_Extraction_Manager.py
